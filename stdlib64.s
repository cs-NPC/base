...
section .text
;
; format string -> color,format,reset,endl
;
printf:
    push rbp
    mov rbp, rsp

    mov rbx, rdi        ; format string pointer <- RDI
    xor r12, r12        ; args indexer
    ;---args RSI RDX RCX R8 R9 R10 order sensitive
.entry_label:
    mov al, [rbx]
    test al, al
    jz .ended
...
.len_arg:
    ...
    jmp .format
;
; mapping args with indexing to dispatch
;
.dispatch_dec:
    cmp r12, 0
    je .rsi_d
